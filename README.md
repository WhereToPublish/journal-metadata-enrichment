...
4. `agent/scripts/run_enrichment.py` loads remote suggestion keys from the `Agent_suggestions` and `Agent_suggestions_processed` Google Sheets tabs, then selects journals from the gap report — all journals across all tabs are processed, sorted by priority (high → medium → low) — and opens one fresh embedded OpenClaw session per journal (inside Docker). It stops when `--max-suggestions` valid suggestions have been written (default: 15) or all journals are exhausted. Before starting each Docker container, the runner runs three host-side pre-fetch steps:
   - **DOAJ API pre-fetch**: retrieves `apc_has_apc`, `apc_price`, `apc_currency`, `boai`, and `license` without Cloudflare issues; included in the prompt as `prefetched_doaj_data`.
   - **OpenAlex pre-fetch**: retrieves `is_oa`, `apc_usd`, `apc_prices`, and `host_organization_name` from the OpenAlex sources API; included in the prompt as `prefetched_openalex_data`.
   - **CrossRef ISSN bypass** (`generate_crossref_issn_suggestions()`): queries the CrossRef journals API (`https://api.crossref.org/journals?query=<name>&rows=5`) with a works API fallback; if an exact or high-confidence title match is found, ISSN suggestion rows (`p-ISSN`, `e-ISSN`, `ISSN-L`) are generated directly — without calling the LLM — and pre-registered in `existing_keys` so the model cannot duplicate them.
   
   After CrossRef ISSN suggestions are resolved, the runner checks whether any actionable gaps remain. If the only remaining gap fields are `Institution` and/or `Institution type` (which the LLM has never been able to resolve in practice), the **LLM is skipped entirely** for that journal (`skip_llm` optimization). This applies to approximately 77% of journals in the backlog, reducing their processing time from ~5 minutes to ~2 seconds. For all other journals, OpenClaw runs as before.
   
   The prompt also includes `scimago_by_issn` whenever an ISSN is known, for resolving Alternative journal name. The `Alternative journal name` gap has the highest priority weight (12), ahead of Business model (10), so it is researched first. The agent is instructed that Business model requires hard evidence (explicit text on the journal page, DOAJ, or prefetched_doaj_data) and that absence of APC mention on a website is **not** evidence that APC = 0; `prefetched_doaj_data.apc_has_apc=false` is accepted as explicit evidence for APC Euros = 0. The per-journal OpenClaw timeout defaults to 900 seconds and is configurable. When `--skip-gap-analysis` is used, the selected gap-report file must already exist.
...
- `agent/scripts/run_enrichment.py`: loads remote deduplication keys from Google Sheets, pre-fetches DOAJ/OpenAlex/CrossRef data before each journal, applies the LLM-skip optimization for Institution-only journals, prompts OpenClaw per journal (via Docker), accepts a configurable per-journal OpenClaw timeout, requires an existing gap report when `--skip-gap-analysis` is used, loops over journals, and records run state.
...
- publisher websites (e.g., Scimago, SAGE, Elsevier) are often protected by Cloudflare WAF and return 403 from Docker datacenter IPs; DOAJ, OpenAlex, and CrossRef APIs (all pre-fetched on the host) bypass this for Business model, APC Euros, and ISSN fields
- journals with only `Institution` / `Institution type` gaps are skipped by the LLM-skip optimization (these fields have never been resolved by the model in practice), reducing processing time from ~5 minutes to ~2 seconds per journal for ~77% of the backlog
