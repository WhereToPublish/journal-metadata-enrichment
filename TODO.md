...
- Inspect the logs in the ./agent/output folder to identify how to improve the performance of the AI agent. Analyze the logs to understand when the AI agent made a wrong decision (both when proposing incorrect values, or when not suggestion the correct one), or when the AI agent did not manage to obtain it. Look for patterns in the logs that could indicate why the AI agent is not performing well, and try to understand the underlying causes of these issues. Based on this analysis, implement improvements by either changing the behavior of the OpenClaw agent (AGENTS.md, HEARTBEAT.md, IDENTITY.md, SOUL.md, TOOLS.md, USER.md, SKILL.md, ...). Test these improvements to see if they lead to better performance.
